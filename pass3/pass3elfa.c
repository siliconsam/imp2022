...
#define CODERELSECTION   3   // the code relocations         - .rela.text
...
#define SWTABRELSECTION  8   // the switch table relocations - .rela.switch
#define TRAPSECTION      9   // the trap table               - .trap
#define TRAPRELSECTION  10   // the trap table relocations   - .rela.trap
...
void initelf(FILE * output)
{
    ...
    if (mainprog != 0)
    {
        ...
        traprelhead.sh_name = newname(".rela.ITRAP$B");
    }
    else
    {
        traphead.sh_name = newname(".ITRAP$D");
        traprelhead.sh_name = newname(".rela.ITRAP$D");
    }
    ...
    setsize(TRAPRELSECTION, (trapseg / 8) * sizeof(Elf32_Rela));
    traprelhead.sh_type = SHT_RELA;
    traprelhead.sh_flags = 0;
    traprelhead.sh_addr = 0;
    traprelhead.sh_offset = dataoffset;
    traprelhead.sh_size = (trapseg / 8) * sizeof(Elf32_Rela);
    traprelhead.sh_link = symtabsection;
    traprelhead.sh_info = trapsection;
    traprelhead.sh_addralign = 4;
    traprelhead.sh_entsize = sizeof(Elf32_Rela);
...
static void puttraptable(FILE *output)
{
    ...
    for (i = 0; i < ns; i++)
    {
        ...
        for (j=0; j < 4; j++)
        {
            ...
            writew32(TRAPRELSECTION, 0);                          // relocation addend for section
        }
    }
...
static void readwrite(FILE *input, FILE *output)
{
    ...
    for(;;)
    {
        ...
        case IF_DATA:   // dataseg offset word
            ...
            writew32(CODERELSECTION, 0);                    // relocation addend for section
            cad += WORDSIZE;
            break;
...
//            writew32(CODESECTION,-4);
        ...
        case IF_SWTWORD:    // switch table entry - actually a label ID
            ...
            writew32(SWTABRELSECTION, 0);                        // relocation addend for section
            swtp += 4;
            break;
